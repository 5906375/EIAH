...
CREATE TABLE public.oraculo_authorities (
 ...
 capabilities text[] NOT NULL CHECK(capabilities <@ ARRAY['execute','read','approve']::text[] AND array_position(capabilities,NULL) IS NULL), PRIMARY KEY(tenant_id,workspace_id,token_ref));
...
CREATE TABLE public.oraculo_executions (
 ...
 tenant_id text NOT NULL, workspace_id text NOT NULL, intent_id text NOT NULL,
 token_ref text NOT NULL, hd text, consumed boolean NOT NULL DEFAULT false, resolved_snapshot jsonb NOT NULL,
 PRIMARY KEY(backend_pid,transaction_id,tenant_id,workspace_id,intent_id));
CREATE TABLE public.oraculo_evaluations (
 tenant_id text NOT NULL, workspace_id text NOT NULL, evaluation_id text NOT NULL,
 intent_id text NOT NULL, payload jsonb NOT NULL, PRIMARY KEY(tenant_id,workspace_id,evaluation_id),
...
BEGIN
 ...
 IF a.token_ref IS NULL OR NOT a.active OR a.expires_at<=clock_timestamp() OR NOT coalesce(cap=ANY(a.capabilities),false)
...
BEGIN
 ...
 IF cap='execute' AND (n.result IS NOT NULL OR n.assessment IS NOT NULL) THEN
 ...
 IF cap='execute' THEN
  INSERT INTO public.oraculo_executions VALUES(pg_backend_pid(),txid_current(),t,w,i,tok,NULL,false,v.snapshot||jsonb_build_object('intent',n.payload,'policy',pol,'visit',v.snapshot->'visit'||jsonb_build_object('revision',v.revision,'state',v.state)));
 END IF;
 RETURN jsonb_build_object('intent',n.payload,'result',n.result,'assessment',n.assessment,'actor',a,
...
BEGIN
 ...
 'GOVERNED_V1',h,jsonb_build_object('reviewerTokenRef',tok,'reviewerBinding',actor,'decision',a));
END $$;

CREATE FUNCTION public.oraculo_finish(t text,w text,tok text,i text,assessment_value jsonb,c5 jsonb,evaluations jsonb DEFAULT '[]'::jsonb) RETURNS void LANGUAGE plpgsql SECURITY DEFINER
SET search_path=pg_catalog,public AS $$
...
BEGIN
 ...
 FOR ev IN SELECT value FROM jsonb_array_elements(evaluations) LOOP
  IF ev->>'schemaVersion' IS DISTINCT FROM 'oraculo.credential-evaluation.v1' OR ev->>'tenantId' IS DISTINCT FROM t
   ...
   OR ev->>'result' IS DISTINCT FROM 'ACCEPTED_FOR_SIMULATION' THEN RAISE EXCEPTION 'EVALUATION_MISMATCH'; END IF;
...
REVOKE ALL ON public.oraculo_fences,public.oraculo_authorities,public.oraculo_scopes,public.oraculo_visits,
 public.oraculo_evaluations,public.oraculo_observations,public.oraculo_intents,public.oraculo_contexts,public.oraculo_executions,public.oraculo_audits FROM PUBLIC;
