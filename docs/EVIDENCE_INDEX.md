...
| Lista governada de casos do workspace (`workspace_case_list`) | `ops/evidence/latest/imob-workspace-case-list-2026-07-07.md` | Evidência real da introdução do intent conversacional `workspace_case_list`, do reuso explícito de `ImobCrmRepository(prisma).listCases(...)` já usado em `GET /imob/cases`, da regressão anti-duplicação de `Próximo passo` no chat IMOB e do status conservador `parcial/evidenciado`. Registra `pass 4 / fail 0` nos testes focados e a limitação residual do `imob.chat.resolve-turn.contract.test.ts` por `ERR_PACKAGE_PATH_NOT_EXPORTED` em `@eiah/core` e `connect EPERM 127.0.0.1:5433`. |
