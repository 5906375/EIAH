...
| Assunto | Arquivo | O que prova |
| --- | --- | --- |
| Job `ci_unit_suite` em `ci.yml` (sem `services:`) para o Grupo B (unit/mock puro) da classificacao D14, ratificada como Opcao 3 hibrida | `ops/evidence/latest/ci-unit-suite-2026-07-07.md` | Prova por execucao real (162 arquivos, `pass:711/fail:0` via `pnpm test:ci-unit-suite`, unico env var `TSX_TSCONFIG_PATH`, sem Postgres/Redis) e por comparacao antes/depois do gate N-12 (`blockingOrphanCount` 210 -> 48, delta -162, exatamente os 162 arquivos do manifesto `scripts/unit-tests-manifest.txt`, nem mais nem menos) que o Grupo B de D14 (177 arquivos) pode rodar em `ci.yml` hoje sem infra; documenta por reproducao real, nao suposicao, os 15 arquivos excluidos do manifesto: 11 com falha real de assercao (pendente de triagem futura, nao corrigidos aqui), 2 reclassificados para o Grupo A (import transitivo de `@repo/db` via `capabilityExecution`/`imobArtifactCapabilities`, gap na heuristica original de D14 que so inspeciona imports diretos), 1 estrutural (`tests/multitenant/tenant-isolation.test.ts`, fora de qualquer workspace package, `@repo/db` nao resolve) e 1 falso positivo do proprio N-12 (`provider-boundary-enforcement.test.ts` ja coberto por `check:provider-boundary` com `cwd=apps/api`, mas o scanner compara caminho relativo ao repo com caminho relativo a `apps/api` no valor do script e nunca casa); registra os 2 achados de gap (heuristica de D14 e falso positivo de N-12) como propostas para frentes futuras, nao corrigidos nesta PR (escopo restrito a wiring do Grupo B). |
>>>>>>> bc3ae85... ci: add unit test suite job for mock-only tests (PR-CI-02)
