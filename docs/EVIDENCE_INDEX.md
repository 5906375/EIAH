...
| Assunto | Arquivo | O que prova |
| --- | --- | --- |
| Alinhamento visual de `/app/imob/chat` ao mock SaaS final | `docs/ops/evidence/latest/phase-9-3-2-product-shell-alignment/phase-9-3-2-product-shell-alignment.md` | Prova por Playwright Chromium (29/29 checks, 5 resoluções) e 50/50 testes unitários que a rota IMOB chat reduziu aparência de Command Center técnico: header global compacto em `py-2` (era `py-4`), logo menor, subtitle/workspaceLabel ocultos nessa rota; sidebar sem descrição técnica; main area sem chip "Chat central preservado" e com `workspaceLabel` de `text-[17px] font-semibold` para `text-[12px] text-slate-500`; painel contextual com título "Resumo do intake" (era "Resumo operacional do intake ativo no chat.") e empty state simplificado. Responsividade da Fase 9.3.1 intacta: textarea visível em todas as 5 resoluções (y=299–769px). Grid `xl:grid-cols-[280px,minmax(0,1fr),360px]` intocado; backend, workers, storage, auth/entitlement intocados. Status: PILOTO CONTROLADO. |
