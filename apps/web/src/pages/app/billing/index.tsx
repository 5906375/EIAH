...
import BillingGuideFooter from "@/components/billing/BillingGuideFooter";
...
const BillingPage: React.FC = () => {
  ...
  const isBillingAdminView = isAdminProfile(experience?.roleProfile);
  ...
  const activeTab = (searchParams.get("tab") || "billing") as "billing" | "economy";
    ...
    <div className="mb-6 inline-flex rounded-full border border-white/10 bg-white/5 p-1">
      <button
        ...
        className={`rounded-full px-4 py-1.5 text-sm font-medium transition ${
          activeTab === "billing" ? "bg-accent/15 text-foreground" : "text-muted-foreground hover:text-foreground"
        }`}
      >
        Billing
      </button>
      <button
        type="button"
        onClick={() => navigate("/app/billing?tab=economy")}
        className={`rounded-full px-4 py-1.5 text-sm font-medium transition ${
          activeTab === "economy" ? "bg-accent/15 text-foreground" : "text-muted-foreground hover:text-foreground"
        }`}
      >
        Economy
      </button>
