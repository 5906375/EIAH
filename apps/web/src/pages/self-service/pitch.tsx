...
import { getAgentConfigBySlug, type GenericAgentConfig } from "./config";
...
export default function SelfServicePitchPage() {
  ...
  const genericConfig = config as GenericAgentConfig;
  ...
  return (
    <div className="space-y-6">
      <SelfServiceNav currentSlug={genericConfig.slug} />
      <AgentFormShell<FormValues>
        agentId={genericConfig.agentId}
        title={genericConfig.title}
        description={genericConfig.description}
        initialValues={initialValues}
        buildRequest={(vals) => genericConfig.buildPrompt(vals)}
      >
        {({ values: formValues, setValue }) => (
          <div className="space-y-4">
            {genericConfig.fields.map((field) => {
