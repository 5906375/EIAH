...
export default function SelfServiceFinNexusPage() {
  ...
  const buildRequest = useCallback((values: FinNexusFormValues) => {
    const lines = [
      ...
      "Contexto Multi-ambiente privado: Toda a sua execução deve ser validada pelo contexto de segurança: ambientePrivadoId, workspaceId, userId. Verifique as permissões de billing.admin ou editor antes de executar qualquer Action que altere o estado financeiro.",
      "Auditabilidade Total: Para cada passo do ciclo de cobrança (criação de plano, conciliação, envio de fatura), emita um registro de auditoria completo (emit_audit_log Tool). O log deve incluir: ambientePrivadoId, workspaceId, runId, traceId, a action executada e o costCents.",
