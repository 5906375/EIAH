...
function toNeedMoreInfoOption(option) {
...
function toNeedMoreInfoField(entry) {
    ...
    const label = typeof entry.label === "string"
        ...
        : typeof entry.prompt === "string"
            ? entry.prompt
            : key;
    const helper = typeof entry.helper === "string"
        ? entry.helper
        : typeof entry.description === "string"
            ? entry.description
            : undefined;
    const placeholder = typeof entry.placeholder === "string" ? entry.placeholder : undefined;
    const rows = typeof entry.rows === "number"
        ? entry.rows
        : typeof entry.minLines === "number"
            ? entry.minLines
            : undefined;
    const declaredType = typeof entry.type === "string" ? entry.type.toLowerCase() : undefined;
    const type = declaredType === "textarea" || declaredType === "select" ? declaredType : "text";
    const required = typeof entry.required === "boolean"
        ? entry.required
...
function collectFieldCandidates(source) {
    const candidates = [];
    if (Array.isArray(source.fields))
        candidates.push(...source.fields);
    if (Array.isArray(source.missingFields))
        candidates.push(...source.missingFields);
    if (Array.isArray(source.missing_fields))
        candidates.push(...source.missing_fields);
    if (isPlainObject(source.details) && Array.isArray(source.details.fields)) {
        candidates.push(...source.details.fields);
    }
    if (Array.isArray(source.required))
        candidates.push(...source.required);
    return candidates;
}
function findNeedMoreInfoNode(payload, depth = 0) {
    if (depth > 5 || !isPlainObject(payload))
        return null;
    const node = payload;
...
function extractNeedMoreInfo(payload) {
    ...
    const title = titleCandidates.find((item) => typeof item === "string");
    if (fields.length === 0 && !message && !title) {
        return null;
    }
    return { title, message, fields };
}
export default function AgentFormShell({ agentId, title, description, initialValues, buildRequest, children, }) {
    const [agent, setAgent] = useState(null);
    const [values, setValues] = useState(initialValues);
    const [extraValues, setExtraValues] = useState({});
    const [estimateStatus, setEstimateStatus] = useState("idle");
    const [estimateCents, setEstimateCents] = useState(null);
    ...
    const [dismissedFollowUpRunId, setDismissedFollowUpRunId] = useState(null);
    ...
    const combineValues = useCallback((base, extra) => ({ ...base, ...extra }), []);
    const mergedValues = useMemo(() => combineValues(values, extraValues), [values, extraValues, combineValues]);
    ...
    const executeRun = useCallback(async (mode, overrides) => {
    ...
    const handleFollowUpSubmit = useCallback((additionalValues) => {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("header", { className: "space-y-3 rounded-3xl border border-white/10 bg-surface/80 p-6", children: [_jsxs("div", { className: "flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs uppercase tracking-[0.3em] text-muted-foreground", children: "Formulario Self-service" }), _jsx("h1", { className: "text-2xl font-display font-semibold text-foreground", children: title }), _jsx("p", { className: "text-sm text-muted-foreground", children: description })] }), _jsxs("div", { className: "flex flex-col items-end gap-2 text-right", children: [_jsx("span", { className: "pill bg-accent/20 text-[10px] uppercase tracking-[0.25em] text-accent", children: agent?.name ?? agentId }), agent?.pricing?.perRunCents !== undefined && (_jsxs("span", { className: "text-xs text-muted-foreground", children: ["A partir de R$ ", (agent.pricing.perRunCents / 100).toFixed(2), " + payload"] }))] })] }), _jsx(EstimateBadge, { status: estimateStatus, cents: estimateCents })] }), _jsxs("section", { className: "space-y-6", children: [_jsxs("div", { className: "rounded-3xl border border-white/10 bg-surface/80 p-6", children: [_jsx("form", { className: "space-y-4", children: children({ values, setValue, updateValues, isSubmitting }) }), error && (_jsx("p", { className: "mt-4 text-xs text-red-300", role: "alert", children: error })), _jsxs("div", { className: "mt-6 flex flex-wrap items-center gap-3", children: [_jsx("button", { type: "button", onClick: () => executeRun("simulate"), className: "rounded-full border border-accent/60 bg-accent/15 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-accent transition hover:border-accent hover:bg-accent/25 disabled:cursor-not-allowed disabled:opacity-60", disabled: isSubmitting, children: isSubmitting ? "Executando..." : "Simular" }), _jsx("button", { type: "button", onClick: () => executeRun("execute"), className: "rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-foreground transition hover:border-accent/40 hover:text-accent disabled:cursor-not-allowed disabled:opacity-60", disabled: isSubmitting, children: isSubmitting ? "Executando..." : "Rodar agora" })] })] }), _jsx(RunStatusCard, { run: lastRun, error: error })] }), _jsx(NeedMoreInfoDialog, { open: isFollowUpOpen && !!followUpDialog, request: followUpDialog, currentValues: mergedValuesAsStringMap, isSubmitting: isSubmitting, onCancel: handleFollowUpCancel, onSubmit: handleFollowUpSubmit })] }));
}
