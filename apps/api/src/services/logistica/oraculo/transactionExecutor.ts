...
import {
  oraculoGateIntentV1Schema, oraculoGateResultV1Schema,
  ...
  await inCi1Transaction(db, intentId, async () => {
    ...
    if (assessment.disposition === "ELIGIBLE_FOR_SIMULATION") {
      ...
      result = oraculoGateResultV1Schema.parse({
        ...
        resultId: randomUUID(), intentId, outcome: "APPLIED", reasonCodes: [], evaluationRefs: [intentId],
    ...
    await db.query("SELECT public.oraculo_finish($1,$2,$3,$4,$5::jsonb,$6::jsonb)",
      [who.tenantId, who.workspaceId, who.tokenRef, intentId, JSON.stringify(assessment), result && JSON.stringify(result)]);
  });
