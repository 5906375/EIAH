...
import { canonicalize, hashContent, hashApprovalContext, hashArtifact } from "./canonicalization.js";
...
export function evaluateResolvedS1(input: unknown): S1Assessment {
  ...
  if (s.policy === "sim-return-hitl.v1") {
    ...
    const currentContext = oraculoGateApprovalContextV1Schema.safeParse(buildS1ApprovalContext(s));
...
export function buildS1ApprovalContext(s: S1Snapshot) {
  ...
  return oraculoGateApprovalContextV1Schema.parse({
