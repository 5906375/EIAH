import { env as processEnv, type ProcessEnv } from "node:process";
...
export function allowUnverifiedWallet(env: ProcessEnv = processEnv) {
  return isExplicitlyEnabled("EIAH_ALLOW_UNVERIFIED_WALLET", env);
}

export function allowPasswordBootstrap(env: ProcessEnv = processEnv) {
  return isExplicitlyEnabled("EIAH_ALLOW_PASSWORD_BOOTSTRAP", env);
}

export function allowEmailPasswordReset(env: ProcessEnv = processEnv) {
  return isExplicitlyEnabled("EIAH_ALLOW_EMAIL_PASSWORD_RESET", env);
}

export function allowGuardrailWarnOnly(env: ProcessEnv = processEnv) {
