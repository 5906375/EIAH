...
test("T-PIPE-1: pipeline completo → draftId válido para contrato Gina 101", () => {
...
test("T-PIPE-5: draft gerado tem actionId = imob.contract.intake", () => {
  const { maskedText } = maskContractPii(LEASE_CONTRACT_GINA_101_SANITIZED_TEXT);
  const extraction = extractLeaseContractFromText(maskedText);
  const classification = classifyImobContract(extraction.lease, maskedText);
  const draft = createDraft({
...
test("T-PIPE-6: draft recuperável por getDraft após criação", () => {
  const { maskedText } = maskContractPii(LEASE_CONTRACT_GINA_101_SANITIZED_TEXT);
  const extraction = extractLeaseContractFromText(maskedText);
  const classification = classifyImobContract(extraction.lease, maskedText);
  const draft = createDraft({
  ...
  const fetched = getDraft(draft.draftId);
  assert.ok(fetched !== null);
  assert.equal(fetched!.draftId, draft.draftId);
});

test("T-PIPE-7: draft não expirado imediatamente após criação", () => {
  const { maskedText } = maskContractPii(LEASE_CONTRACT_GINA_101_SANITIZED_TEXT);
  const extraction = extractLeaseContractFromText(maskedText);
  const classification = classifyImobContract(extraction.lease, maskedText);
  const draft = createDraft({
...
test("T-PIPE-8: PII em texto sintético é mascarado antes do draft", () => {
...
test("T-PIPE-10: valores monetários são inteiros (sem float) no draft", () => {
  const { maskedText } = maskContractPii(LEASE_CONTRACT_GINA_101_SANITIZED_TEXT);
  const extraction = extractLeaseContractFromText(maskedText);
  const classification = classifyImobContract(extraction.lease, maskedText);
  const draft = createDraft({
...
test("T-PIPE-12: requiresConfirmation = true em todas as etapas do pipeline", () => {
  const { maskedText } = maskContractPii(LEASE_CONTRACT_GINA_101_SANITIZED_TEXT);
  const extraction = extractLeaseContractFromText(maskedText);
  const classification = classifyImobContract(extraction.lease, maskedText);
  const draft = createDraft({
